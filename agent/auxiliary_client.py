...
# Default timeout for OpenAI client calls (seconds).
# The OpenAI SDK default is 600s (10 min), which causes long hangs on
# Windows when DNS/network is flaky. 120s is sufficient for most API calls.
_DEFAULT_OPENAI_TIMEOUT = 120
...
def _resolve_api_key_provider() -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    for provider_id, pconfig in PROVIDER_REGISTRY.items():
        ...
        if pool_present:
            ...
            return OpenAI(api_key=api_key, base_url=base_url, timeout=_DEFAULT_OPENAI_TIMEOUT, **extra), model
...
def _try_openrouter() -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    if pool_present:
        ...
        return OpenAI(api_key=or_key, base_url=base_url,
                       timeout=_DEFAULT_OPENAI_TIMEOUT,
...
def _try_custom_endpoint() -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    if custom_mode == "codex_responses":
        real_client = OpenAI(api_key=custom_key, base_url=custom_base, timeout=_DEFAULT_OPENAI_TIMEOUT)
        return CodexAuxiliaryClient(real_client, model), model
    return OpenAI(api_key=custom_key, base_url=custom_base, timeout=_DEFAULT_OPENAI_TIMEOUT), model
...
def _try_codex() -> Tuple[Optional[Any], Optional[str]]:
    ...
    real_client = OpenAI(api_key=codex_token, base_url=base_url, timeout=_DEFAULT_OPENAI_TIMEOUT)
...
def resolve_provider_client(
    ...
    if provider == "openai-codex":
        if raw_codex:
            ...
            raw_client = OpenAI(api_key=codex_token, base_url=_CODEX_AUX_BASE_URL, timeout=_DEFAULT_OPENAI_TIMEOUT)
    ...
    if provider == "custom":
        if explicit_base_url:
            ...
            if "api.kimi.com" in custom_base.lower():
                ...
                from hermes_cli.models import copilot_default_headers
                extra["default_headers"] = copilot_default_headers()
            client = OpenAI(api_key=custom_key, base_url=custom_base, timeout=_DEFAULT_OPENAI_TIMEOUT, **extra)
    ...
    try:
        ...
        if custom_entry:
            ...
            if custom_base:
                ...
                client = OpenAI(api_key=custom_key, base_url=custom_base, timeout=_DEFAULT_OPENAI_TIMEOUT)
                client = _wrap_if_needed(client, final_model, custom_base)
                logger.debug(
