...
def _try_custom_endpoint() -> Tuple[Optional[Any], Optional[str]]:
    ...
    if custom_mode == "codex_responses":
        real_client = OpenAI(api_key=custom_key, base_url=_clean_base, timeout=_DEFAULT_OPENAI_TIMEOUT, **_extra)
    ...
    if custom_mode == "anthropic_messages":
        ...
        try:
            ...
            return OpenAI(api_key=custom_key, base_url=_clean_base, timeout=_DEFAULT_OPENAI_TIMEOUT, **_extra), model
        return (
...
def resolve_provider_client(
    ...
    if provider == "custom":
        if explicit_base_url:
            ...
            client = OpenAI(api_key=custom_key, base_url=_clean_base, timeout=_DEFAULT_OPENAI_TIMEOUT, **extra)
            ...
            if custom_base:
                ...
                if entry_api_mode == "anthropic_messages":
                    try:
                        ...
                        client = OpenAI(api_key=custom_key, base_url=_fb_clean, timeout=_DEFAULT_OPENAI_TIMEOUT, **_fb_extra)
                        return (_to_async_client(client, final_model, is_vision=is_vision) if async_mode
                                else (client, final_model))
                ...
                client = OpenAI(api_key=custom_key, base_url=_clean_base2, timeout=_DEFAULT_OPENAI_TIMEOUT, **_extra2)
