...
def _sanitize_subprocess_env(base_env: dict | None, extra_env: dict | None = None) -> dict:
    ...
    if _profile_home:
        ...
        # On Windows, also set USERPROFILE for consistency
        if _IS_WINDOWS:
            sanitized["USERPROFILE"] = _profile_home
...
# Use windows_compat for cross-platform path handling.
from tools.windows_compat import get_sane_path_str, join_path_list
...
def _make_run_env(env: dict) -> dict:
    ...
    # Use os.pathsep-aware check and join for Windows compatibility
    if "/usr/bin" not in existing_path.split(os.pathsep) and "System32" not in existing_path:
        run_env["PATH"] = join_path_list(existing_path, _SANE_PATH) if existing_path else _SANE_PATH
